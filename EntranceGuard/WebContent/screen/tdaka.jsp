				...
				<div class="col-md-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<div class="text-muted bootstrap-admin-box-title">打卡记录</div>
							<div
								class="text-muted bootstrap-admin-box-title pull-right text-warning"
								style="cursor: pointer" onclick="add()">添加打卡记录</div>
						</div>
...
<script type="text/javascript">
	function search() {
		...
		$.ajax({
					...
					url : "/EntranceGuard/tdaka.htm?name=" + name,
					dataType : "json",
					success : function(data) {
						if (data.result == "error") {
							...
							//alert(data.message);
							var dataObj = data.map.tDaka;
							$("#tdakatable  tr:not(:first)").empty();
							$('#name').val("");
	...
	function add() {
		layer.open({
			...
			area : [ '500px', '400px' ],
			//弹出框地址
			content : '/EntranceGuard/addtdaka.htm',
			//点击关闭按钮事件
			end : function() {
			}
		});

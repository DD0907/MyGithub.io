				...
				<div class="col-md-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<div class="text-muted bootstrap-admin-box-title">系别</div>
							<div
								class="text-muted bootstrap-admin-box-title pull-right text-warning"
								style="cursor: pointer" onclick="add()">添加系别</div>
						</div>
...
<script type="text/javascript">
	function search() {
		...
		$.ajax({
			...
			url : "/EntranceGuard/tdept.htm?name=" + name,
			dataType : "json",
			success : function(data) {
				if (data.result == "error") {
					...
					//alert(data.message);
					var dataObj = data.map.tXibie;
					$("#tdepttable  tr:not(:first)").empty();
					$('#name').val("");

			...
			<div class="collapse navbar-collapse"
				...
				<div class="navbar-form navbar-left" role="search">
					<div class="form-group">
						<input type="text" class="form-control" placeholder="请输入优惠券查询"
							id="name">
					</div>
				...
				<div class="col-md-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<div class="text-muted bootstrap-admin-box-title">优惠券</div>
							<div
								class="text-muted bootstrap-admin-box-title pull-right text-warning"
								style="cursor: pointer" onclick="add()">添加优惠券</div>
						</div>
						<div class="bootstrap-admin-panel-content">
							<table class="table table-striped table-striped table-bordered table-hover" id="coupontable">
								<thead>
	...
	function search() {
		...
		$
				.ajax({
					...
					url : "/taobaos/coupon.htm?name=" + name,
					dataType : "json",
					success : function(data) {
						...
						if (data.result == "error") {
							...
							//alert(data.message);
							var dataObj = data.map.coupon;
							$("#coupontable  tr:not(:first)").empty();
							$('#name').val("");
							$
									.each(
											dataObj,
														...
														.append($td
																...
																.html(
																		'<input type="button" value="修改"  class="btn btn-primary" onclick="update('
																				...
																				+ item.id
																				+ ');" />'));
												$("#coupon").after($tr);//最后要把内容放入要绑定的地方
											});
						}
					}
				});

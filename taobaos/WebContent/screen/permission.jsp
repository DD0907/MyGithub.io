			...
			<div class="collapse navbar-collapse"
				...
				<div class="navbar-form navbar-left" role="search">
					<div class="form-group">
						<input type="text" class="form-control" placeholder="请输入权限名进行查询"
							id="name">
					</div>
				...
				<div class="col-md-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<div class="text-muted bootstrap-admin-box-title text-primary">权限</div>
							<div
								class="text-muted bootstrap-admin-box-title pull-right text-warning"
								style="cursor: pointer" onclick="add()">添加权限</div>
						</div>
...
<script type="text/javascript">
	function search() {
		...
		$
				.ajax({
					...
					url : "/taobaos/permission.htm?name=" + name,
					dataType : "json",
					success : function(data) {
						if (data.result == "error") {
							...
							//alert(data.message);
							var dataObj = data.map.permission;
							$("#permissiontable  tr:not(:first)").empty();
							$('#name').val("");
							//alert(dataObj)
							$
									.each(
														...
														.append($td
																...
																.html(
																		'<input type="button" value="修改"  class="btn btn-primary" onclick="update('
																				...
																				+ item.id
																				+ ');" />'));
												$("#permission").after($tr);//最后要把内容放入要绑定的地方
											});
						}
					}
				});
	...
	function update(id) {
		layer.open({
			...
			area : [ '500px', '400px' ],
			//弹出框地址
			content : '/taobaos/updatePermission.htm?id=' + id,
			//点击关闭按钮事件
			end : function() {
			}
		});
	...
	function add() {
		layer.open({
			...
			area : [ '500px', '400px' ],
			//弹出框地址
			content : '/taobaos/addPermission.htm',
			//点击关闭按钮事件
			end : function() {
			}
		});
	...
	function del(id) {
		$.ajax({
			type : "POST",
			contentType : 'application/json',
			url : "/taobaos/deletePermission.htm?id=" + id,
			dataType : "json",
			success : function(data) {
